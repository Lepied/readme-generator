...
export const CHAIN_STEP_PROMPTS = {
    ...
    intro: `이 프로젝트의 이름, 한 줄 슬로건, 그리고 간단한 소개를 작성하세요.
[Project Context]
{projectContext}
...
## 📖 프로젝트 소개
{소개 내용}`,
    features: `이 프로젝트의 핵심 기능을 **사용자/기획자 관점**에서 5개 내외의 글머리 기호로 요약하세요.
...
3. 너무 세세한 수치(예: rollDistance) 설명은 생략하고 "구르기 거리 조절 가능" 정도로 요약하세요.`,
    ...
    installation: `이 프로젝트({projectType})를 로컬에서 실행하기 위한 정확한 설치/실행 가이드를 작성하세요.
...
[제약] 
1. "Git 다운로드 하세요" 같은 뻔한 말은 빼세요.
2. Unity/Unreal같은 게임프로젝트의 경우 **"1. 에디터 설치 -> 2. 프로젝트 열기 -> 3. 씬 실행"** 딱 3단계로 요약하세요.
3. 웹 프로젝트는 명령어 코드블록만 제공하세요.`,

    usage: `이 프로젝트의 사용법을 작성하세요.
...
[제약]
1. 설명은 짧게, 핵심 조작키나 엔드포인트 위주로 작성하세요. 
2. 게임이면 조작키, 웹이면 접속 URL, 라이브러리면 예제 코드를 포함하세요.`,

    structure: `이 프로젝트의 폴더 구조와 아키텍처를 설명하세요.
...
3. "Scripts 폴더는 게임 로직을 담당합니다" 같은 뻔한 말 대신, 어떤 디자인 패턴이 쓰였는지 설명하세요.`
};